...
**Core Innovation**: Requirement decomposition → issue creation via `ccw issue create`. Issues stored in `.workflow/issues/issues.jsonl` (single source of truth). Wave/dependency info embedded in issue tags (`wave-N`) and `extended_context.notes.depends_on_issues`. issue-devpipeline consumes issues directly by ID or tag query.

**Core workflow**: Requirement Understanding → Strategy Selection → Context Collection (optional) → Decomposition + Issue Creation → Validation → issue-devpipeline Handoff
...
│     └─ Next steps: issue-devpipeline full execution / wave-by-wave / view│
│                                                                          │
└─────────────────────────────────────────────────────────────────────────┘
```
...
3. **Post-Completion Options**
   ...
   if (!autoYes) {
     Ask_question({
       questions: [{
         ...
         multiSelect: false,
         options: [
           { label: "Execute with issue-devpipeline", description: `启动 issue-devpipeline 执行全部 ${issueIds.length} 个 issues` },
   ...
   | Selection | Action |
   |-----------|--------|
   | Execute with issue-devpipeline | `/codex:issue-devpipeline "${issueIds.join(' ')}"` |
   | Execute first wave | Filter issues by `wave-1` tag, pass to issue-devpipeline |
...
- issue-devpipeline handoff available via issue IDs
...
6. **issue-devpipeline integration**: Issues created follow standard issues-jsonl-schema, directly consumable by issue-devpipeline via issue IDs and tags
