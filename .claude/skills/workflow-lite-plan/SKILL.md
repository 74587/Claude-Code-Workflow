...
<purpose>
Planning pipeline: explore → clarify → plan → confirm → handoff to workflow-lite-execute.
Produces exploration results, a structured plan (plan.json), independent task files (.task/TASK-*.json), and hands off to workflow-lite-execute for implementation.
</purpose>
...
| LP-5 | Build executionContext → Skill("workflow-lite-execute") | handoff (Mode 1) |
...
**IMPORTANT**: LP-Phase 3 is **planning only** — NO code execution. All execution happens in LP-Phase 5 via workflow-lite-execute.
...
if (workflowPreferences.autoYes) {
  ...
  userSelection = AskUserQuestion({
    questions: [
      ...
      {
        question: "Code review after execution? (runs in workflow-lite-execute)",
        header: "Code Review",
        multiSelect: false,
        options: [
...
**CRITICAL**: lite-plan NEVER executes code directly. ALL execution goes through workflow-lite-execute.
...
TodoWrite({ todos: [
  ...
  { content: `LP-Phase 5: Handoff → workflow-lite-execute`, status: "completed" },
...
- [ ] Handoff to workflow-lite-execute via Skill("workflow-lite-execute") with executionContext
- [ ] No code execution in planning phases -- all execution deferred to workflow-lite-execute
</success_criteria>
